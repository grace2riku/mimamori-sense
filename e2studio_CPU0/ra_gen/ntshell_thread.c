...
void ntshell_thread_create(void);
static void ntshell_thread_func(void *pvParameters);
void rtos_startup_err_callback(void *p_instance, void *p_data);
void rtos_startup_common_init(void);
extern uint32_t g_fsp_common_thread_count;

const rm_freertos_port_parameters_t ntshell_thread_parameters = { .p_context =
		(void*) NULL, };
...
#if 1
	ntshell_thread = xTaskCreateStatic(
#else
...
#if 1
	if (NULL == ntshell_thread) {
		rtos_startup_err_callback(ntshell_thread, 0);
	}
#else
                    if (pdPASS != ntshell_thread_create_err)
                    {
                        rtos_startup_err_callback(ntshell_thread, 0);
                    }
                    #endif
...
static void ntshell_thread_func(void *pvParameters) {
	...
	ntshell_thread_entry(pvParameters);
}
