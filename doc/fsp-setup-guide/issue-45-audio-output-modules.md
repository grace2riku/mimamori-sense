...
> ## ⚠ 本書には実機実測で否定された記述があります
>
> Issue #46 の実機検証で、本書の以下の記述が誤りであることが判明しました。
> **該当箇所は `doc/fsp-setup-guide/issue-46-audio-clock-fix.md` が正となります。**
>
> | 本書の記述 | 実態 |
> |---|---|
> | 1-2節 / 2-4節 / 5-4節: 「SSIE の Internal AUDIO_CLK は **GPT1** の GTIOC1A」 | **GPT2**。GPT1 は動作しているが未使用（削除対象） |
> | 2-4節: Bit Clock Divider `/1` で fs 16,009 Hz | **`/24`** が必要。fs は 16,276 Hz |
> | 1-8-3節 / 10-4節: 「J41 は初期設定で 1-2 / 3-4 とも短絡済み。**作業不要**」（UM 表2 由来） | **実機は 3-4 が開放だった**。目視確認が必須 |
>
> **J41 3-4（P405 = SSITXD0 → DA7212 DATIN）が開放だと、I2S 再生データが
> コーデックに1ビットも届きません。DA7212 で音が出ないときは最初にここを確認してください。**
>
> なお DA7212 内蔵ビープはコーデック内部生成で **DATIN を通らない**ため、
> ビープが鳴っても I2S データ経路の証明にはなりません（詳細は #46 手順書 6-2節）。
