...
| マーカー | 対応するステップ |
|---------|----------------|
| `/content/.cleaning_applied` | Step 2.5 データセットクリーニング |
| `/content/.hardneg_applied` | Step 2.6 hard negative mining |
| `/content/.anchors_updated` | Step 4 アンカー再計算 |

いずれもセル冒頭で既存マーカーを削除するため、前回実行の残骸は効かない。
