...
`TIME_CTRL_ERR_HW` が実際に返るのは、**読み出した RTC の値が壊れていた場合のみ**。
この事実は `time_ctrl.h` の `@retval` と `time_ctrl.c` 冒頭にも明記する。

### 読み出し値の検証は 2 段構え（PR #216 レビュー指摘 P2 / 2026-08-28 追記）

FSP の `rtc_bcd_to_dec()` は `(上位ニブル) * 10 + (下位ニブル)` を計算するだけで、
ニブルが 0-9 に収まっているかを検査しない（`r_rtc.c:1725-1729`）。このため
**下位ニブルが 0xA-0xF の壊れた生値が、範囲内の 10 進値としてデコードされてしまう**:

| 生値 | `rtc_bcd_to_dec()` | 範囲検査 |
|---|---|---|
| `RSECCNT = 0x1A` | 1×10 + 10 = **20** | 20 ≤ 59 なので**通る** |
| `RSECCNT = 0x20` | 2×10 + 0 = 20 | 通る（正当な値） |

デコード後の 20 からは両者を区別できないため、範囲検査だけでは不十分。
そこで検証を 2 段に分ける:

1. `rtc_raw_bcd_is_valid()` — **生レジスタ**のニブルが BCD として妥当か。
   検査対象は FSP が実際にデコードに使うバイトそのものとする（`RHRCNT` は FSP と同じく
   `0x3F` でマスク、`RWKCNT` は BCD ではなく 3bit バイナリなのでバイト全体を 6 以下と比較）
2. `time_is_valid()` — デコード後の値が日時として成立するか（2 月 30 日・19 月など
   桁の組み合わせの不正）

適用箇所は `time_ctrl_get()` と `time_ctrl_init()` の読み出し直後（いずれもロック区間内）。
`time_ctrl_init()` で弾かない場合、壊れた時刻がそのまま `tk_set_utc()` に流れ込む。
