...
## 16. 追い指摘: GPIO 制御 2 本が調査対象から抜けていた（2026-08-29、Issue #222）

### 指摘内容

§13 / §15 の「ソフト側の切り分け完了」表は **GLCDC が使う 30 本の LCD_GRAPHICS ピン**しか
見ていない。LCD には GPIO で制御する 2 本があり、`display pins` / `display signals` の
どちらからも読めない盲点になっていた。

| ネット名 | ピン | 役割 | 制御箇所 |
|---|---|---|---|
| `DISP_RESET` | P606 | LCD パネル ＋ GT911 タッチコントローラのリセット（共用） | `glcdc_port.c` `glcdc_lcd_reset()` |
| `DISP_BLEN` | P514 | バックライトイネーブル | `glcdc_port.c` `glcdc_port_backlight_control()` |

**`DISP_RESET` が Low のままなら、GLCDC のレジスタもピン波形も完全に正常に見えたまま、
パネルは全面白になる。** これは §15 で観測した状態と**ソフトウェアだけでは区別できない**。
したがって §15 の「原因はハードウェア側」という結論は、この 1 本を確認するまで
成立していなかった。

### 手動測定の結果（白画面状態）

`mr` コマンドで PmnPFS を直読みした。アドレスは `R_PFS_BASE(0x40400800) + port*0x40 + pin*4`。

```
mimamori>mr 0x40400998 4
0x40400998: 0x00000407      ← P606 (DISP_RESET)
mimamori>mr 0x40400978 4
0x40400978: 0x00000407      ← P514 (DISP_BLEN)
```

| ビット | 値 | 意味 |
|---|---|---|
| bit0 `PODR` | 1 | ソフトは Hi を出力している（リセット解除済み／バックライト ON） |
| bit1 `PIDR` | **1** | **パッドの実レベルが High**。外部から Low に引かれていない |
| bit2 `PDR` | 1 | 出力方向のまま |
| bit[11:10] `DSCR` | 1 (MID) | `ra_gen/pin_data.c` の `IOPORT_CFG_DRIVE_MID` と一致 |
| bit16 `PMR` | 0 | GPIO のまま（周辺機能に奪われていない） |
| bit[28:24] `PSEL` | 0 | 同上 |

`PIDR` はパッドの入力レベルなので、「MCU は High を出しているが基板側で Low に
引かれている」ケースなら `0x00000405` になる。**そうなっていない。**

### 独立した裏付け

同じ拡張ボード上の**タッチコントローラが正常動作する**ことも確認した
（`touch read` を 4 本指でタッチしながら実行し、FT5X06 形式の 4 点の座標が正しく返る）。
`DISP_RESET` は GT911 と共用で、リセット保持中の I2C デバイスは ACK を返せないため、
これは PmnPFS の読み値とは独立した P606 解除の裏付けになる。

### 結論

**2 本とも正常。§15 の結論（原因は MCU のピンより先）は変わらない。**
ただし、同じ見落としが再発しないよう、この 2 本を恒久的に `display pins` へ載せた
（Issue #222）。切り分け表を更新すると:

| 層 | 結果 |
|---|---|
| （§13 の 9 行はそのまま） | |
| **GPIO 制御ピン（P606 リセット / P514 バックライト）** | **2/2 正常（`0x00000407`）** |
| **タッチコントローラ（P606 共用、I2C）** | **正常応答（4 点座標）** |

### 学んだこと

**「ペリフェラルが使うピン」を全部見ても、「そのペリフェラルの外から効くピン」は
見えない。** 切り分けの網は、対象ブロックのピン一覧ではなく、
**その出力に影響しうる線の一覧**で張る必要がある。
