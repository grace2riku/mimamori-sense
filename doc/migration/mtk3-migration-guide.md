...
- 採用根拠の要点:
  ...
  3. 案A で問題が出た場合は `lv_thread_init` を失敗させると LVGL が同期描画へ自動退避する
     ため、案C 相当へ段階的に縮退可能。
