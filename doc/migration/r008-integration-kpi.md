...
| # | 確認項目 | 手順 / 期待値 | 結果 |
|---|----------|---------------|------|
| I-05 | LED 点滅（blink） | LED1(Blue) が **500ms ごとにトグル**（点灯 500ms ＋消灯 500ms ＝ **フル点滅周期は約 1 秒**）。`blink_task` はトグル後に `tk_dly_tsk(500)`（`usermain.c:133-138`） | **NG（原因特定済み・修正は未マージ）**<br>2026-07-26: LED1 が**明らかに速い**。追加観測（リセット後 3 秒間は正常、以降だけ速い）から原因を特定。**AI 推論インジケータが同じ P600 を叩いていた**（→ 4.4 D-04）。<br>⚠ **本リビジョンには修正が含まれていない**。`application_config.h:42` は `ENABLE_INFERENCE_RUNNING_LED (1)` のままで、このブランチをビルドしても症状は再現する。修正は **PR #177（未マージ）**。**#177 マージ後にリビルドして再測定するまで NG のまま**とする |
...
#### D-04: LED1 の点滅周期が仕様より速い（I-05）→ **原因特定済み・修正は未マージ** #175 / PR #177
...
**修正（PR #177・未マージ）**: `ENABLE_INFERENCE_RUNNING_LED` を `(0)` にして既定で無効化する。
⚠ **本リビジョン（PR #171 のブランチ）には修正が含まれていない**。`ai_application/application_config.h:42`
は `(1)` のままであり、このブランチをビルドしても症状は再現する。
推論の実行状況は `ai status`（Inference count）で確認できるため LED を占有する必要がない。
オシロで推論周期を観測したいときだけ `(1)` に戻す運用とし、副作用をコメントに明記した。

**実機での確認事項（PR #177 の受け入れ判定）**: **PR #177 を取り込んだ状態で**リビルドし、
**リセットから 3 秒経過しても LED1 が 1 秒周期を保つ**こと。併せて `ai status` の Inference count が
従来どおり増加すること。**この再測定が済むまで I-05 は NG のまま**とする。
