...
> **本書の値の読み方（測定状況：2026-07-26 更新）**
>
> 本書には**実機で測定済みの値**と**未測定の欄**が混在する。区別は各表の「結果」「測定値」「判定」欄で行う。
>
> - **測定済み（2026-07-26、EK-RA8P1 実機でユーザーが実施）**: I-05〜I-12、I-18、KPI-01。
>   結果欄に日付を付して記載してある。これらは**実測値であり推測値ではない**。
> - **未測定**: I-01〜I-04、I-13〜I-17、KPI-02、B-01〜B-03。結果欄は **「未測定」** のまま。
> - **KPI-05（2026-09-26 更新）**: Issue #237で警報連携を実装し測定可能。実機動作確認OKの通知は受領済みだが、通知遅延の実測値は未記録・未判定。
> - **R-008 スコープ外**: KPI-03 / KPI-04（測定手順の整備までを R-008 とし、実施は F-003 側）。
>
> 残作業の一覧は 9 章を参照。
>
> **捏造禁止（本書作成方針）**: 本書作成者（Claude）は実機を持たないため、**実機でしか得られない値を
> 自ら埋めることはしない**。未測定欄は「未測定」と明記し、推測値・捏造値を入れない。
> 上記の測定済みの値は、すべてユーザーが実機で取得しシェル出力とともに報告したものである。
>
...
| # | 確認項目 | 手順 / 期待値 | 結果 |
|---|----------|---------------|------|
| I-05 | LED 点滅（blink） | LED1(Blue) が **500ms ごとにトグル**（点灯 500ms ＋消灯 500ms ＝ **フル点滅周期は約 1 秒**）。`blink_task` はトグル後に `tk_dly_tsk(500)`（`usermain.c:133-138`） | **NG（修正済み・実機再測定待ち）**<br>2026-07-26: LED1 が**明らかに速い**。追加観測（リセット後 3 秒間は正常、以降だけ速い）から原因を特定。**AI 推論インジケータが同じ P600 を叩いていた**（→ 4.4 D-04）。修正は **PR #177 でマージ済み**（`ENABLE_INFERENCE_RUNNING_LED` を `(0)` へ）。<br>⚠ **リビルドしての再測定が未実施のため判定は NG のまま**とする。再測定で 1 秒周期を確認できた時点で OK とすること |
...
| I-15 | 優先度逆転 | ⚠ **前版の手順は成立しない**（下記「I-15 の正しい競合経路」参照）。**正しい確認対象**: NT-Shell(12) の `lvgl` サブコマンドが `lv_lock()` を取る（`usrcmd.c:403,447,500`）一方、その lock を保持しうるのは `lvgl_task`(14) と描画スレッド(13) であり、**高優先の NT-Shell が低優先の保持者を待つ**構図になる。手順: カメラ表示＋AI を並走させた高負荷状態で `lvgl status` / `lvgl mem` を繰り返し実行し、**応答が長時間ブロックしないこと**を確認する。LVGL mutex は `TA_INHERIT`（`lv_os_mtkernel.c` の `tk_cre_mtx`、`:180`）のため保持者が 12 へ引き上げられ、中間優先の描画スレッド(13) に割り込まれて待ちが延びることは無いはず | 未測定 |
...
| I-18 | AI 並走時の描画 FPS 低下 | R-007 実機で **AI 並走時カメラ表示 20 FPS（CPU 60%）** を観測済み（手順書 更新履歴 2026-06-12 R-007）。30fps KPI との照合は KPI-01 で実施 | **確認（再現）**<br>2026-07-26: 全機能同時動作下で `camera display` の `Display FPS = 20`。AI は並走中（`ai status` の Inference count が増加）。R-007 の懸念が**そのまま再現**。`display dbuf` の `Underflow count = 0` は**GLCDC の読み出しが間に合っていること**を示すのみで、描画側の SDRAM 帯域競合は否定できない（→ 4.4 D-05）。→ KPI-01 は未達 |

> **I-15 の正しい競合経路（Codex レビュー指摘により訂正、2026-07-26）**
>
> 前版は I-15 を「低優先 ai(15) が mutex 保持中に高優先タスクが待たされ続けないこと。
> `lv_lock`（lvgl=14）が長時間 ai(15) に占有されないか観察」としていたが、**この競合は起こり得ない**。
>
> `lv_lock()` を呼び出すのは **`lv_os_mtkernel.c`（OSAL 実装本体）と `usrcmd.c` の 2 ファイルのみ**で
> （`grep -l "lv_lock()" src/**/*.c` で確認）、**`ai_inference_thread_entry.c` および
> `ai_application/` 配下は LVGL の mutex を一切取得しない**。したがって前版の手順では
> `TA_INHERIT` を一度も動作させないまま「合格」になってしまう。
>
> **実際に優先度継承が効くのは NT-Shell ⇄ LVGL の競合**である（優先度は数値が小さいほど高い）:
>
> | 役割 | タスク | 優先度 | lv_lock との関係 |
> |---|---|---|---|
> | 待つ側（高優先） | ntshell | **12** | `lvgl` サブコマンドが `lv_lock()` を取る（`usrcmd.c:403`, `:447`, `:500`） |
> | 中間優先 | dave2d / swdraw 描画スレッド | 13 | 継承が無ければ保持者を割り込んで待ちを延ばす |
> | 保持する側（低優先） | lvgl_task | 14 | `lv_timer_handler` 等の内部で lock を保持 |
>
> 継承が無ければ「NT-Shell(12) が lvgl_task(14) の lock を待つ間に描画スレッド(13) が lvgl_task を
> 割り込み続ける」という**無制限の優先度逆転**が成立する。`TA_INHERIT`（`lv_os_mtkernel.c:180` の
> `tk_cre_mtx`）は保持者を 12 へ引き上げるため 13 に割り込まれなくなり、待ちが有界になる。
> I-15 はこの経路を実際に踏ませて確認すること。
...
> **副産物（μT-Kernel 時間基準の粗い検証）**: `display dbuf` の測定窓は
> `tk_dly_tsk(1000)`（`glcdc_port.c:492`）である。理論値 70.30Hz に対し実測 71 だった。
>
> ⚠ **訂正（Codex レビュー指摘、2026-07-26）**: 前版はここから「測定窓 ≒ 1.010 秒であり
> `+TIMER_PERIOD` と厳密に一致する」と結論したが、これは**誤り**。カウント値は整数であり、
> 70.30Hz の信号を**ちょうど 1.000 秒**サンプルしても開始位相次第で 70 にも 71 にもなる。
> 1 回の整数カウントから測定窓を 1.010 秒と特定することはできない。
>
> この測定から言えるのは**量子化誤差 ±1 パルスを考慮した範囲**だけである:
> 70.30 × T ∈ [70, 72] → **T ∈ 約 [0.996, 1.024] 秒**（≒ −0.4% 〜 +2.4%）。
> したがって「時間基準が正確」とまでは言えず、**数十%オーダーの粗い狂いが無いことを示すに留まる**。
>
> **精密に検証するには**、より長い測定区間か独立した高分解能クロックが必要:
> - `display dbuf` の `[Counters] Vsync Count`（累積値）を**ストップウォッチで測った長時間**
>   （例 10 分）の前後で読み、差分 ÷ 実時間で実効リフレッシュレートを求める（分解能が約 600 倍になる）
> - μT-Kernel の時間基準そのものを測るなら `led 2 blink 1000` を実行し、
>   **トグル 60 回の所要時間をストップウォッチで計測**する（正常なら約 60 秒）

#### D-04: LED1 の点滅周期が仕様より速い（I-05）→ **原因特定・修正済み（実機再測定待ち）** #175 / PR #177
...
**修正（PR #177 でマージ済み）**: `ENABLE_INFERENCE_RUNNING_LED` を `(0)` にして既定で無効化した
（`ai_application/application_config.h`）。
推論の実行状況は `ai status`（Inference count）で確認できるため LED を占有する必要がない。
オシロで推論周期を観測したいときだけ `(1)` に戻す運用とし、副作用をコメントに明記した。

**実機での確認事項（#175 のクローズ判定）**: 修正を含む main をリビルドし、
**リセットから 3 秒経過しても LED1 が 1 秒周期を保つ**こと。併せて `ai status` の Inference count が
従来どおり増加すること（推論自体に影響がないこと）。**この再測定が済むまで I-05 は NG のまま**とする。
...
Codex レビュー指摘（2026-07-26）により判明した**解釈の誤り**。コードの不具合ではなく
本書の記述側の問題だが、KPI-01（#176）の調査を誤った方向へ導く危険があるため記録する。

前版は `display dbuf` の `Underflow count = 0` から「**SDRAM 帯域不足ではない**」と結論して
いたが、これは**言い過ぎ**である。

underflow がカウントされるのは `RM_LVGL_PORT_EVENT_UNDERFLOW` 受信時
（`glcdc_port.c:866-878`）で、その意味はコメントにあるとおり
「the display controller could not read framebuffer data fast enough」＝
**GLCDC が表示用にフレームバッファを読み出す経路が間に合わなかった**ことのみを示す。

ダブルバッファ構成では、**GLCDC はフロントバッファを走査し続けられる**ため、
その裏で Dave2D の描画・CPU によるコピー・AI のメモリアクセスがバックバッファ側で
SDRAM 帯域に律速されていても、**underflow は 1 度も発生しない**。

正しい言い方:

- ✅ `Underflow count = 0` → **表示側（GLCDC 読み出し）の帯域は足りている**
- ❌ `Underflow count = 0` → SDRAM 帯域全体が十分である／帯域はボトルネックではない

**KPI-01 の原因調査では SDRAM 帯域競合を候補から外さず、別途プロファイルすること**
（#176 に反映済み）。
...
KPI 定義の出典は `doc/product-requirements.md` の **3.1 定量的指標（KPI）** の表。

> ⚠ **参照の書き方について（Codex レビュー指摘、2026-07-26）**: 前版は PRD を行番号（`:102`、
> `:105-109`、`:175-181`）で参照していたが、PRD 冒頭への追記で行がずれ、**S-011 など無関係な要件を
> 指してしまっていた**。要件文書は改訂で行が動きやすいため、本書からの参照は
> **節番号＋表の行ラベル（項目名）という安定したアンカー**で行う。

| ID | KPI | 合格基準 | 出典（要件） | 測定方法 | 測定値 | 判定 |
|----|-----|----------|--------------|----------|--------|------|
| KPI-01 | フレームレート（F-001/F-002） | **30fps 以上**（表示の滑らかさ） | PR 3.1「F-001/F-002 フレームレート」 | ⚠ **現状のファームには物理的な描画完了フレーム数を数える計装がない**（下記「KPI-01 で使える指標の限界」参照）。当面は次の複合判定とする: ①`camera display` の `Display FPS`（`camera_display.c:337-338`）＝**表示更新要求**レート、②`display dbuf` の `Vsync Rate`（`glcdc_port.c:498-499`。**期待値は約 70.3Hz**＝ピクセルクロック 60MHz ÷ (1344×635)。→ 4.4 D-03）と `Underflow count`（**0 であること**＝GLCDC の読み出しが間に合っている。⚠ **描画側の SDRAM 帯域競合までは否定できない**→ 4.4 D-05）、③**目視でカクつき・ティアリングが無いこと**、④参考にキャプチャ FPS = `camera status`（`camera_framebuffer.c:170`）。**ただし④は約 2 倍に過大計上される**ので実値は表示の 1/2 として読む（→ 4.4 D-01）。**②の `Render FPS` は毎 Vsync カウントのため合格判定に使わない**（限界参照） | **2026-07-26**<br>①Display FPS = **20**<br>②Vsync 71Hz / Underflow **0**<br>④camera status FPS 65（実値 ≒ 32.5） | **未達（NG）**<br>①が 30fps に届かない。R-007 の I-18（AI 並走時 20FPS）が再現。②の Underflow=0 が示すのは **GLCDC の読み出しが間に合っていること**のみで、**描画側の SDRAM 帯域競合は未検証**（→ 4.4 D-05）。→ 6. の調整方針を適用（#176） |
| KPI-02 | AI 推論時間（F-003） | **5ms 以内** | PR 3.1「F-003 AI推論時間」 | `ai time` の NPU inference 時間。⚠ **現状は整数 ms 切り捨て表示のみで 5ms 境界（5.000〜5.999ms）を判定できない**（下記「KPI-02 の計測粒度」参照）。厳密判定には μs/サイクル単位の計装追加が必要（別 Issue）。R-007 単独では 5ms 表示を確認済み（更新履歴 R-007）→ R-008 は**全機能同時動作下で再測定** | 未測定 | 未判定 |
| KPI-03 | 転倒検出精度（F-003） | **検出率 90% 以上** | PR 3.1「F-003 転倒検出精度」 | 転倒シナリオを N 回実施し、**試行ごとに「検出できた／できなかった」の 2 値で記録**する。検出率 = **検出できた試行数 ÷ N**。⚠ **イベント総数を試行数で割ってはならない**（下記「KPI-03/04 の数え方」参照）。1 試行の判定手順: ⓪**試行前に必ず `fall reset` を実行**（または `fall status` で `State: NORMAL` を確認）→ ①`Confirmed total` を記録 → ②シナリオ実施 → ③再度 `fall status` を読み、**増分が 1 以上なら「検出」**（増分が 2 以上でもその試行は 1 とカウント）。⚠ **⓪を省くと検出漏れが偽装される**（下記「試行前リセットが必須な理由」参照）。データセット・回数は F-003 側で定義 | 未測定 | **R-008 スコープ外**<br>手順整備のみ。実施は F-003 側（→ 9. 「KPI-03/04 のスコープ」） |
| KPI-04 | 誤検出率（F-003） | **5% 以下** | PR 3.1「F-003 誤検出率」 | 非転倒シナリオ（歩行・着座・しゃがみ等）を M 回実施し、**試行ごとに「誤検出が 1 回以上起きた／起きなかった」の 2 値で記録**する。誤検出率 = **誤検出が起きた試行数 ÷ M**。判定手順は KPI-03 と同じ（**⓪試行前に `fall reset`** → `Confirmed total` の増分が 1 以上なら誤検出ありとして 1 カウント）。⚠ 数え方と試行前リセットの注意は下記参照。`fall log` の遷移履歴も併せて記録すると原因分析に使える | 未測定 | **R-008 スコープ外**<br>手順整備のみ。実施は F-003 側（→ 9.）<br>⚠ 2026-07-26 の I-11 で `fall log` 32 エントリ中の大半が `NORMAL↔SUSPECTED` の往復（AR 1.31〜1.66 / score 0.52〜0.79）。撮影対象が管理されていないため判定不可だが、**誤検出率が高い可能性**があり F-003 側で優先確認 |
| KPI-05 | 転倒検出（確定）→**警報音**通知時間（F-004） | **10 秒以内** | PR 3.1「転倒検出→通知時間」 | 起点＝`fall_detection_update()` 内で `CONFIRMED` に達した時点（`e2studio_CPU0/src/fall_detection_logic.c:161`）、終点＝実際の警報音の立ち上がり。Issue #237で測定可能。画面表示遅延・要求受付は代用しない。下記「KPI-05 と F-004 警報連携」参照 | 未測定（実測値未記録） | 未判定 |

### KPI 測定上の注意（コード事実に基づく）

> 以下のクールダウンと reset に関する記述は、R-008 検証当時の実装・測定条件を残したもの。
> Issue #237 で転倒確定を姿勢回復まで保持する方式へ変更し、クールダウンを廃止する。
> 新しい状態遷移・解除条件は [Issue #237 設計](../design/issue-237.md) を参照する。

- **KPI-03/04 の数え方（イベント数ではなく試行ごとの 2 値で数える）**: 検出率・誤検出率は
  必ず**試行単位の 2 値（検出あり／なし）**で集計する。「`Confirmed total` の増加イベント総数 ÷ 試行数」
  で計算してはならない。
  - 理由（コード事実）: 確定後は `COOLDOWN` に入り `s_cooldown_counter` を数え切ると
    **`NORMAL` へ復帰する**（`fall_detection_logic.c:200-213`）。よって被験者が転倒姿勢を
    クールダウン時間より長く保つと、**1 回の試行中に `confirmed_count` が複数回加算されうる**
    （`:164` で `s_stats.confirmed_count++`）。実測でもクールダウンは 1000 フレーム
    （2026-07-26 の `fall log`: frame 25966 → 26966）＝ 約 30 秒相当だった。
  - イベント数で割ると次の破綻が起きる:
    - **検出率が 100% を超える**（1 試行で 2 回確定すれば 2 とカウントされる）
    - **1 試行での重複検出が、検出できなかった試行を埋め合わせてしまう**（見かけ上の精度が上がる）
    - 誤検出率も同様に、1 シナリオでの連続誤検出が実態より悪く／良く出る
  - したがって記録票は「試行 No / 検出あり(1)・なし(0) / 参考: `Confirmed total` 増分」の形にし、
    率の分子には**2 値の合計**を使う。
- **試行前リセットが必須な理由（省くと検出漏れが偽装される）**: KPI-03/04 の各試行の前に
  **必ず `fall reset` を実行する**（または `fall status` で `State: NORMAL` を確認する）。
  - 理由（コード事実）: `fall_detection_update()` の状態遷移は `s_state` による分岐で、
    **`FALL_STATE_COOLDOWN` の分岐はカウンタを減算して `NORMAL` へ戻すだけで候補評価を行わない**
    （`fall_detection_logic.c:200-213`）。したがって**クールダウン中に起きた本物の転倒は
    確定しない**（`confirmed_count` が増えない）。
  - クールダウンは 1000 フレーム（2026-07-26 実測、約 30 秒相当）と長いため、
    **試行間隔がこれより短いと、次の試行が自動的に「検出漏れ」として記録されてしまう**。
    KPI-03 の検出率が実力より低く出る（KPI-04 は逆に誤検出を見逃す方向に働く）。
  - `fall reset` は `s_state` / `s_consecutive_count` / `s_cooldown_counter` をクリアする一方で
    **累積カウンタ（`confirmed_count` 等）は保持する**（`fall_detection_logic.c` の
    `/* Update stats but keep cumulative counters */`）。よって**増分方式の判定と併用できる**。
...
- **KPI-05 と F-004 警報連携（2026-09-26 更新）**: KPI-05「転倒検出→通知時間 10 秒以内」は実際の警報音通知を対象とする。
  - **履歴**: R-008検証当時は警報連携が未実装のため測定不可だった。Issue #237で実装され、現在は測定可能。
  - **現在のコード**: `fall_detection_update()` の確定遷移（`e2studio_CPU0/src/fall_detection_logic.c:161`）後、同 `:188` から `alarm_sound_sync_fall_state()` を呼び、現在の状態に応じて緊急警報を要求する（`e2studio_CPU0/src/audio_alarm.c:637`）。既存警報タスクが適用・再試行する（同 `:961,1570`）。確定コールバックへのハンドラ登録を測定の前提にしない。状態・解除・失敗時の詳細は [Issue #237 設計](../design/issue-237.md) を参照。
  - **測定方法**: 確定状態を書いた時刻と実際の音声出力の立ち上がりを共通の時間軸で記録し、差を測る。GPIOによる時刻マーカーと音声波形など、測定方法・音量・ミュート・バス競合条件・使用イメージを記録する。要求受付や画面表示の時刻を発音の代わりにしない。[実機確認手順](../../scripts/issue237/README.md)を参照。
  - **測定状況**: ユーザーから実機動作確認OKの通知は受領済み。通知遅延の実測値は未提供のため、KPI-05は未測定・未判定。製品KPIの10秒以内とIssue #237の通常条件での100ms以内という実装目標は区別する。
  - **起点の定義**: 起点は転倒確定であり、物理的な転倒発生ではない。転倒発生から警報までを評価する場合は、確定までの検出遅延を別途測り、エンドツーエンド遅延として内訳を記録する。
...
確保を伴うか、描画中に実行しうるか）は未検証**である。→ **調査 Issue #178**（調査項目・調査方法・
対処案を記載）。
...
> ⚠ **実機動作から B-01 は推論できない**（Codex レビュー指摘、2026-07-26）
>
> 2026-07-26 に実機で全機能が動作したが、**これは B-01（クリーンビルド成功）の証拠にならない**。
> ボードには**以前ビルドされたイメージが書かれたまま**の可能性があり、そのイメージが
> 現在のソース・現在のツールチェイン・Debug 構成でビルドされたものである保証がないため。
> B-02（オーバーフローしないこと）も同様に、リンク時のメモリ使用量レポートを見なければ
> 「余裕がどれだけあるか」は分からない。
>
> **B-01〜B-03 は次の記録が揃うまで「未測定」のままとする**:
> - B-01: クリーンビルド（一度 Clean してからのビルド）が成功した旨と、
>   **ビルド構成が Debug であること**が分かるビルドログ
> - B-02: リンカのメモリ使用量サマリ（Flash / RAM の使用量と上限）
> - B-03: **その成果物を書き込んだ直後**の起動ログ（→ I-01〜I-04）
...
30fps は製品要求（`doc/product-requirements.md` 3.1「F-001/F-002 フレームレート」）であるため、性能改善は
**R-008 とは別に対応する（→ #176）**。判明している事実:

- `display dbuf` の `Underflow count = 0` → **GLCDC の読み出しは間に合っている**
  （表示側のフレームバッファ読み出しが枯渇する事象は無い）。
  ⚠ ただしこれは**描画側の SDRAM 帯域競合を否定しない**（→ 4.4 D-05）
- R-007 の I-18「AI 並走時カメラ表示 20 FPS（CPU 60%）」がそのまま再現した
- キャプチャ側は約 32.5fps 出ており、カメラ入力はボトルネックではない
- **ボトルネックの所在は未特定**。候補は描画・転送側（LVGL / Dave2D）、AI との CPU 競合、
  および描画側の SDRAM 帯域競合。調整方針は 6. を参照
