...
`p_leds[1]`=P303 のみを書き、P600 には触れない。時間基準については D-03 の副産物として
...
| KPI-03 | 転倒検出精度（F-003） | **検出率 90% 以上** | PR 3.1 `:107` | 転倒シナリオを N 回実施し、**試行ごとに「検出できた／できなかった」の 2 値で記録**する。検出率 = **検出できた試行数 ÷ N**。⚠ **イベント総数を試行数で割ってはならない**（下記「KPI-03/04 の数え方」参照）。1 試行の判定手順: ①試行前に `fall status` の `Confirmed total` を記録 → ②シナリオ実施 → ③再度 `fall status` を読み、**増分が 1 以上なら「検出」**（増分が 2 以上でもその試行は 1 とカウント）。データセット・回数は F-003 側で定義 | 未測定 | **R-008 スコープ外**<br>手順整備のみ。実施は F-003 側（→ 9. 「KPI-03/04 のスコープ」） |
| KPI-04 | 誤検出率（F-003） | **5% 以下** | PR 3.1 `:108` | 非転倒シナリオ（歩行・着座・しゃがみ等）を M 回実施し、**試行ごとに「誤検出が 1 回以上起きた／起きなかった」の 2 値で記録**する。誤検出率 = **誤検出が起きた試行数 ÷ M**。判定手順は KPI-03 と同じ（`Confirmed total` の増分が 1 以上なら誤検出ありとして 1 カウント）。⚠ 数え方の注意は下記「KPI-03/04 の数え方」参照。`fall log` の遷移履歴も併せて記録すると原因分析に使える | 未測定 | **R-008 スコープ外**<br>手順整備のみ。実施は F-003 側（→ 9.）<br>⚠ 2026-07-26 の I-11 で `fall log` 32 エントリ中の大半が `NORMAL↔SUSPECTED` の往復（AR 1.31〜1.66 / score 0.52〜0.79）。撮影対象が管理されていないため判定不可だが、**誤検出率が高い可能性**があり F-003 側で優先確認 |
