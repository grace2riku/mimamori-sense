...
接続先とJ41の訂正は[音声出力の設定資料](../fsp-setup-guide/issue-45-audio-output-modules.md#L3)を参照してください。

### 3.3 RTCバックアップ電池

![RTCバックアップ電池の接続](images/rtc-battery.jpeg)

使用電池は **CR2032（3.0V）** です。製作者から、100円ショップで購入できるCR2032を使用するとの回答を受けています。

提供写真では、電池ホルダーのケーブルを基板上の **J36（BATTERY）** に接続しています。ホルダーの型番とピンごとの極性は提出前に確定します。写真ではコネクタで隠れたピンの極性を確定できないため、接続済みの機材では写真の配線を維持し、新規組み立て時は使用キットの端子仕様と照合してください。
...
- LCD・カメラ・電池ホルダーの詳細型番、およびJ36のピンごとの極性。
