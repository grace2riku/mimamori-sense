...
UI compile flags. All objects are rebuilt into Debug/issue206/<run>.
"""
...
def build(run):
    ...
    out = (DEBUG / 'issue206' / run).resolve()
    assert out.is_relative_to((DEBUG / 'issue206').resolve())
    out.mkdir(parents=True, exist_ok=False)
    link = (DEBUG / 'mimamori_sense_CPU0.elf.in').read_text(encoding='utf-8-sig')
    # Enforce the approved instrumentation even when e2 studio has not refreshed
    # its response file after the .cproject link-option change.
    wrapped_isrs = ('ssi_txi_isr', 'ssi_int_isr')
    for name in wrapped_isrs:
        link += ' -Wl,--wrap=' + name
    ...
    for name in ('rtc_boot_diag', 'rtc_backup'):
        ...
        if obj not in objects:
            objects.append(obj)
            link += ' ./' + obj
    assert len(objects) == 1600, f'Unexpected source set: {len(objects)} objects'
    inputs = {}
    for obj in objects:
    ...
    manifest = {'full_recompile': True, 'object_count': len(objects), 'source_sha256': inputs,
                'wrapped_isrs': wrapped_isrs, 'link_response_sha256': digest(out / 'link.in'),
