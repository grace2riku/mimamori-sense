...
PR #205 の `audio_start()` の実例:

| 要素 | 根拠 | 最悪 |
|---|---|---|
| アンミュート試行3回 × `da7212_mute()` 内の4転送（2レジスタ × read+write） | `audio_port.c:626-639` / `da7212.c:859-865,347-365` | 12転送 × バスロック待ち1000ms（`i2c_bus0.h:81`） |
| リトライ間ディレイ × 2 | `audio_port.c:635-638`（`AUDIO_UNMUTE_RETRY_MS` = 5） | 10 ms |
| 失敗後の `audio_stop()`（さらに4転送＋アイドル待ち70ms） | `audio_port.c:641-643` | 4転送 × 1000ms + 70 ms |

「リトライ回数×タイムアウト」なら 3秒。実際は **16秒超**で、呼び出し元の予算を5倍以上見誤る。
